...
The : means makes a list from the bottom number to the top number that
...
x\[x\<10\] returns the elements of x that are less than 10

```{r}
x^2
```

x\^2 returns the square of the values of x

```{r}
x[x<12 & x>7]
```

x\[x\<12 & x\>7\] returns the values that satisfy both conditions of x
being less than 12 but greater that 7
...
-1:2 means each element in the sequence is incremented by 1 starting
from -1 until it reaches 2

```{r}
(-1):2
```

(-1):2 generates the sequence of integers from -1 to 2

```{r}
-(1:2)
```

This negates each element of the sequence of integers from 1 to 2
...
It contains 20 rows. The extra variable represents extra hours of sleep.
The group variable indicates different groups or conditions. The ID
variable represents unique identifiers for the individuals in the
dataset.
...
```{r}
colnames(sleep)[colnames(sleep) == "BodyWt"] -> "Body_weight"
```
...
This plot is showing the difference in total sleep time between
different species and primates.
...
```{r}
colnames(sleep)[colnames(sleep) == "Body_weight"] -> "BodyWt"
```
...
```{r}
my_dataset <- read.csv("http://raw.githubusercontent.com/jsgosnell/CUNY-BioStats/master/datasets/sleep.csv", stringsAsFactors = T)
```
...
```{r}
library(ggplot2)
sleep <- read.csv("http://raw.githubusercontent.com/jsgosnell/CUNY-BioStats/master/datasets/sleep.csv", stringsAsFactors = TRUE)
ggplot(sleep, aes(x = factor(Primate), y = TotalSleep, fill = factor(Primate)))
  geom_boxplot() 
  labs(x = "Primate", y = "Total Sleep Time")
 theme_minimal()
```
...
```{r}
model <- lm(TotalSleep ~ BrainWt, data = sleep)
summary(model)
```

The p-value for the model indicates the significance of the relationship
between TotalSleep and BrainWt. Because the p-value is 0.00578, which is
below the significance level of 0.05, it implies that the relationship
