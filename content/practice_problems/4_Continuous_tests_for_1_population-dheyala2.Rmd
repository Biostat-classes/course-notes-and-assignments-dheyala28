...
sample_mean <- mean(strawberries)
sample_sd <- sd(strawberries)

standard_error <- sigma / sqrt(length(strawberries))

z_value <- (sample_mean - mu) / standard_error

p_value <- 2 * (1 - pnorm(abs(z_value)))

result <- list(z_value = z_value, p_value = p_value)

print(result)

```

**The p-value, 0.14, is greater than 0.05, suggesting that there is
evidence that i fail to reject the null hypothesis. this suggests that
the u=205.**
...
**The p-value, 0.2023, is greater than 0.05, suggesting that there is
evidence that i fail to reject the null hypothesis. this suggests that
the u=205.**
...
SIGN.test <- function(x, md) {
  ...
  p_value <- 2 * pbinom(statistic, n_positive + n_negative + n_equal, 0.5)
...
result <- SIGN.test(strawberries, md)

print(result)
```

**The p-value, 0.34, is greater than 0.05, suggesting that there is
evidence that i fail to reject the null hypothesis. this suggests that
the u=205.**
...
a\) H~0~: The sex ratio is 50% females. H~A~: The sex ratio is not 50%
females.

b\) H~0~: The sex ratio is 80% females. H~A~: The sex ratio is not 80%
females.
...
I'll conduct a one-sample t-test to determine whether the observed mean
undulation rate significantly differs from the hypothesized value of 1.4
Hz. This is justified because we are comparing the mean undulation rate
of the observed data to a known hypothesized value. The t-test assesses
whether the difference between the sample mean and the hypothesized mean
is statistically significant, considering the variability within the
sample. It's appropriate for this scenario since we have a small sample
size and wish to compare it to a population mean.
...
t_test <- t.test(undulation_rates, mu = null_mean)
print(t_test)

```

H~0~: The observed mean undulation rate is 1.4 Hz H~A~: The observed
mean undulation rate is different than 1.4 Hz

The p-value is 0.8335, which is greater than 0.05, suggesting we fail to
reject the null hypothesis. The observed mean undulation rate is 1.4 Hz.
...
suggesting that *males training at the AIS have a lesser weight than the
...
sport <- read.table("http://www.statsci.org/data/oz/ais.txt", header = TRUE, stringsAsFactors = TRUE)

male_weights <- sport$weight[sport$sex == "M"]


mean_weight_australian_male <- 85.9

mean_diff <- function(data, indices) {
  mean_weight_athlete <- mean(data[indices])
  mean_weight_athlete - mean_weight_australian_male
}
```
...
bootstrap_results <- boot(male_weights, statistic = mean_diff, R = 10000)
```
...
data_url <- "http://www.statsci.org/data/oz/ais.txt"
population_unif <- read.table(data_url, header = TRUE)


ggplot(population_unif, aes(x = Wt)) +
  geom_histogram(color = "black", fill = "skyblue", bins = 20) +
  labs(title = "Weight Distribution of Australian Athletes",
       x = "Weight (kg)",
       y = "Frequency")
...
for (i in 1:number_of_simulations) {
  sampling_experiment$observed_mean[i] <- mean(sample(population_unif$value, 50, replace = FALSE))
}
