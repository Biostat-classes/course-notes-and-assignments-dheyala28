...
```{r}
data_clean <- na.omit(sleep)
```

```{r}
ggplot(data_clean, aes(x = BrainWt, y = TotalSleep)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Relationship between Brain Weight and Total Sleep",
       x = "Brain Weight (g)",
       y = "Total Sleep (hours)") +
  theme_minimal()
```
...
```{r}
data <- mutate(sleep, Taxa = ifelse(Primate == "yes", "Primate", "Non-Primate"))
```

```{r}
ggplot(data, aes(x = BrainWt, y = TotalSleep, color = Taxa)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Relationship between Brain Weight and Total Sleep",
       x = "Brain Weight (g)",
...
```{r}
ggplot(sleep[sleep$Primate == "Y",], aes(x = TotalSleep)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") + 
  #below here is ylabel, xlabel, and main title
  ylab("Frequency") +
  xlab("Total Sleep (hours)") +
  ggtitle("Distribution of Total Sleep Time for Primates") +
  #theme sets sizes, text, etc
  theme(axis.title.x = element_text(face="bold", size=13), 
        ...
        plot.background = element_rect(fill="gray"),
        legend.background = element_rect(fill="gray"),
        strip.text.x = element_text(size = 10, colour = "purple"))
```
...
```{r}
levels(sleep$Primate)<- c("Non-Primate","Primate") 
```

```{r}
sd(sleep[sleep$Primate == "Non-Primate", "TotalSleep"], na.rm = T) / 
  sqrt(length(na.omit(sleep[sleep$Primate == "Non-Primate", "TotalSleep"])))
```

```{r}
sd(sleep[sleep$Primate == "Primate", "TotalSleep"], na.rm = T) / 
  sqrt(length(na.omit(sleep[sleep$Primate == "Primate", "TotalSleep"])))
```
...
ggplot (sleep_by_primate
  ...
  geom_col(size = 3) +
  geom_errorbar(aes(ymin=TotalSleep-ci, ymax=TotalSleep+ci), size=1.5) +
  ylab("Total sleep (hours per day")+ggtitle("Sleep differences between Primates and Non-Primates")+
...
A 95% confidence interval suggests the range of values that you can be
95% certain contains the true mean of a population.
